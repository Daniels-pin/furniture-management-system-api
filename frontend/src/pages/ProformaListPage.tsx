      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="text-sm font-bold">Proforma #{r.proforma_number}</div>
        ...
        {showActions ? (
          <div className="mt-3 grid grid-cols-2 gap-2" onClick={(e) => e.stopPropagation()}>
            <Button variant="secondary" className="w-full" onClick={() => nav(`/proforma/${r.id}`)}>
              Open
            </Button>
            <Button variant="danger" className="w-full" onClick={() => setDeleteTarget(r)}>
              Delete
            </Button>
          </div>
        ) : (
      ...
      <Card>
        ...
        <div className="space-y-3 md:hidden">
          {loading ? (
            ...
            <div className="text-sm text-black/60">No proforma invoices yet.</div>
          ) : (
            rows.map((r) => <ProformaCard key={r.id} r={r} />)
          )}
        </div>
