...
export function QuotationFormPage() {
  ...
  async function discardDraft() {
    setRecoverLoading(true);
    try {
      await draftsApi.remove("quotation");
    } catch {
      // ignore
    }
    try {
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void draftsApi
        .upsert("quotation", payload as any)
        .catch(() => {
          // keep silent; local fallback remains
        });
    }, 1200);
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const d = await quotationApi.get(editId);
        if (!alive) return;
        if (d.status === "converted") {
          toast.push("error", "This quotation was already converted.");
          nav(`/quotations/${editId}`);
          return;
        }
  ...
  function buildPayload(saveAsDraft: boolean): QuotationPayload {
  ...
  async function submit(saveAsDraft: boolean) {
    ...
    try {
      ...
      const out = isEdit ? await quotationApi.update(editId, payload) : await quotationApi.create(payload);
      toast.push(
        "success",
        isEdit ? (saveAsDraft ? "Draft saved." : "Quotation updated.") : saveAsDraft ? "Draft created." : "Quotation created."
      );
      if (!isEdit) {
        // Clear autosaved draft on successful submission (draft or finalized).
        try {
          await draftsApi.remove("quotation");
        } catch {
          // ignore
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="space-y-4">
          <div className="text-sm text-black/70">
            You have an unfinished <span className="font-semibold">Quotation</span>. Do you want to continue where you left off?
          </div>
      ...
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
        <div>
          <div className="text-2xl font-bold tracking-tight">{isEdit ? "Edit quotation" : "New quotation"}</div>
        </div>
        <Button variant="secondary" onClick={() => nav(isEdit ? `/quotations/${editId}` : "/quotations")}>
          Cancel
        </Button>
      </div>
