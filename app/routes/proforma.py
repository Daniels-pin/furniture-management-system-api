          ...
          <div style="padding:18px 22px;border-bottom:1px solid #e5e5e5">
            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:18px;font-family:Inter,Arial,sans-serif">
              ...
              <div style="min-width:240px;text-align:right">
                <div style="display:inline-block;background:#111;color:#fff;padding:10px 14px;font-weight:800;letter-spacing:0.22em">PROFORMA INVOICE</div>
                <div style="margin-top:10px;font-size:13px;color:#111">
                  <div><span style="color:#666">Proforma Number:</span> <strong>#{escape(p.proforma_number)}</strong></div>
...
def list_proforma(
    ...
    user_ids = {int(p.created_by) for p in rows if p.created_by is not None}
...
@router.post("/proforma", response_model=ProformaDetailResponse)
def create_proforma(
    ...
    db.flush()
    db.refresh(p)
    items = db.query(models.ProformaItem).filter(models.ProformaItem.proforma_id == p.id).all()
    store_computed_totals(
        p,
        items,
    ...
    db.commit()
    db.refresh(p)
    return _proforma_to_detail(db, p)


@router.get("/proforma/{proforma_id}", response_model=ProformaDetailResponse)
def get_proforma(
    proforma_id: int,
    db: Session = Depends(get_db),
...
@router.put("/proforma/{proforma_id}", response_model=ProformaDetailResponse)
def update_proforma(
    proforma_id: int,
    ...
    if p.status == "converted":
        raise HTTPException(status_code=400, detail="Cannot edit a converted proforma")
...
@router.patch("/proforma/{proforma_id}/finalize", response_model=ProformaDetailResponse)
...
@router.post("/proforma/{proforma_id}/print")
def record_proforma_print(
    proforma_id: int,
    db: Session = Depends(get_db),
    user=Depends(require_role(["admin", "showroom"])),
):
...
@router.post("/proforma/{proforma_id}/convert-to-invoice")
def convert_proforma_to_invoice(
    proforma_id: int,
    ...
    log_activity(
        ...
        meta=presales_to_invoice_activity_meta(
            source="proforma",
            document_number=p.proforma_number,
            actor_user=user,
